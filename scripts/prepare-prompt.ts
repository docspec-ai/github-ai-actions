...
async function fetchPRData(
  ...
  // Get the diff using git or GitHub API
  let diff = "";
  try {
    // Try to get diff using git (if we have the commits)
    if (pr.commits?.nodes && pr.commits.nodes.length > 0) {
      ...
      try {
        ...
        console.warn("Failed to get diff via git, trying GitHub API");
        ...
        // GitHub API doesn't return full diff in compare endpoint
        // We'll need to construct it or use a different approach
        diff = `Diff for ${data.files?.length || 0} files:\n`;
        for (const file of data.files || []) {
          diff += `${file.status} ${file.filename}\n`;
          if (file.patch) {
            diff += `${file.patch}\n`;
          }
        }
      ...
      // Fallback: use GitHub API comparison
      const { data } = await octokit.rest.repos.compareCommitsWithBasehead({
        owner,
        repo,
